const sketch = function(p) {
  ...
  const canvasPadding = window.innerWidth < 500 ? 0 : 20;
  ...
  const iterationName = 'publictest';
   ...
   */
    ...
    // Add event listeners to control trial sequence flow
  ...
  */
  p.mousePressed = function () {
    if (!modelIsActive && p.isInBounds() && modelLoaded) {
      x = p.mouseX;
      y = p.mouseY;
      if (!startX) {
        humanStrokes = 0;
  ...
  p.mouseReleased = function () {
    if (!modelIsActive && p.isInBounds() && modelLoaded) {
      ...
      if (trialDict[currTrial]['condition']!='solo' && trialDict[currTrial]['phase']=='collab') {
        // Change the turn right away, so that
        // the user can't accidentally draw a second time on the canvas before model gets a chance.
	      robotTurn();
      }
    }
  }
  ...
  */
  ...
  function showFeedback(emoji, str) {
  ...
  function updateInstructions() {
      ...
      // If final trial reached, then end gracefully
      if (currTrial>=numClasses*numReps - 1) { // that is, if previous trial was the final one, hence minus 1
          // reopen splash page to thank person for playing
          header = "That's all! Thanks for playing! To learn more about this research, please visit: https://github.com/cogtoolslab/collab-draw.";
          paragraph = " ";
          buttonLabel = 'Done';
  ...
  function getTargetProb () {
    ...
    const topGuess = orderedList[0];

    if (targetInd == 0) {
      showFeedback('🎉🏆', 'Wow, great sketch - "' + S2C[targetName] + '" is exactly what I would have guessed!');
    } else if (targetInd < 5) { // Top 5
      showFeedback('🎉', 'Great job! It looks most like a "' + topGuess + '" to me, but "' + S2C[targetName] + '" was still one of my top guesses! (' + targetIndPlusOne + rankSuffix + ', to be exact)');
    } else if (targetInd < 10) { // Top 10
      showFeedback('🤔', 'Nice work. It looks most like a "' + topGuess + '" to me, but "' + S2C[targetName] + '" is the ' + targetIndPlusOne + rankSuffix + ' guess I would have made.');
    } else { // Everything else
      showFeedback('😅', "Good try, but I'm not sure I would be able to tell this sketch was a " + S2C[targetName] + ".");
    }
  }
  ...
  function getNudgeClass () {
    ...
    reopenSplash(header,paragraph,buttonLabel);
    showFeedback('👋', 'How about turning it into a ' + nudgeClass + ' ?');
